...
const int _CATEGORY = 1;
const int _AMOUNT = 2;
const int _DATE = 3;
const int _ACCOUNT = 4;
...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    //filter = 0 - Income
    arrayCategories = [CategoryData getCategoriesToDisplay:[appDelegate getDBPath] filterBy:0];
    ...
    //Unmark all the previous marked rows
...
- (void) additionalViewDidLoad
{
    ...
    [self setSubViewScreen:_CATEGORY];
    ...
    _titleBarCategoryTableView.backgroundColor = [UIColor colorWithRed:0.7761 green:0.8625 blue:0.7636 alpha:1.0];
    ...
    //_titleBarAccountTableView.backgroundColor = [UIColor grayColor];
    _titleBarAccountTableView.backgroundColor = [UIColor colorWithRed:0.7761 green:0.8625 blue:0.7636 alpha:1.0];
    ...
    //Prepare to play the sound when user creates a new income
    player = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:[[NSBundle mainBundle] pathForResource:@"coinsToTable" ofType:@"mp3"]] error:nil];
...
{ 
    [self dismissKeyboard];
    [self setSubViewScreen:_DATE];
}

- (IBAction)accountButtonPressed:(UIButton *)sender 
{
    [self dismissKeyboard];
    [self setSubViewScreen:_ACCOUNT];
}
...
{
    UIAlertView *alertCancel = [[UIAlertView alloc] initWithTitle:@"Cancel add income" message:@"Confirm cancel?" delegate:self cancelButtonTitle:@"Yes" otherButtonTitles:@"No", nil];
    alertCancel.tag = 1;
    [alertCancel show];
}
...
{
    BOOL _isValidate = [self validateNewIncome];
    
    if (_isValidate == TRUE)
    {
        BOOL _isSaved = [self saveNewIncome];
